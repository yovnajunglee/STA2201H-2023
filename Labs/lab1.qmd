...
dtot <- left_join(dm, dsize, by = c("Year", "Age")) %>% mutate(DeathsF = (Female*PopF), DeathsM = (Male*PopM)) %>% group_by(Year) %>% summarise(TD_M = sum(DeathsM, na.rm = T), TD_F = sum(DeathsF, na.rm =T), PopTotal_M = sum(PopM, na.rm = T), PopTotal_F = sum(PopF, na.rm = T)) 

dw <- dtot %>% mutate(weighted_f = TD_F/PopTotal_F, weighted_m = TD_M/PopTotal_M)
